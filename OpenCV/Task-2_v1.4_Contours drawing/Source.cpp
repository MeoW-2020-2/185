...
int main()
{
	...
	for (int i = 0; i < contours.size(); i++) {
		mu[i] = moments(contours[i], false);  //нахождение центра масс с помощью moments (contours.size - кол-во контуров)
	}
	...
	for (int i = 0; i < contours.size(); i++) {
		...
		drawContours(drawing, contours, i, color, 2, 8, hierarchy, 0, Point());  //рисование контуров(изображение поверх которого будут контуры, найденные контуры, индекс контура, цвет, толщина, тип соединения точек вектора, информация об иерархии контуров, индекс отображаемого слоя (0 - только выбранный контур, 1 - выбранный контур и все его дочерние контуры), сдвиг точек контура);
